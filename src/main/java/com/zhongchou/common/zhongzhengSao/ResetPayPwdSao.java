...
import com.zhongchou.common.zhongzheng.util.TimeoutException;
	//重置支付密码
	public class ResetPayPwdSao extends BaseSao {
		Logger logger=Logger.getLogger(ResetPayPwdSao.class);
		String functionId = "G0000009";

	public ResetPayPwdSao(String req_ssn) {
	...
	public Map receivEncryptData(String content){
		...
		try {
			...
			logger.info("ResetPayPwdSao receivEncryptData response ("+functionId+") :\r\n"+Log4jUtil.output(encryptData));
	...
	public Map setEncryptData(Map setData){
		logger.info("ResetPayPwdSao setEncryptData start ");
		...
		logger.info("ResetPayPwdSao setEncryptData encryptDatarequest("+functionId+") :\r\n"+Log4jUtil.output(scMap.getMap().toString()));
